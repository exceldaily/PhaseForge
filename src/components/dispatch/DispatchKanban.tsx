...
import { Plus, ArrowLeft, AlertCircle, Package, Clock, Eye, EyeOff } from 'lucide-react'
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-shrink-0 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 px-5 py-3">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <h1 className="text-sm font-semibold text-slate-900 dark:text-white">{board.name}</h1>
...
function KanbanColumn({
  column, cards, vendors, onCardClick, onAddCard, userRole,
}: {
...
function DispatchCardCompact({
  ...
  return (
    ...
    >
      ...
      {/* SC # */}
      {card.sc_number && (
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">SC# {card.sc_number}</p>
      )}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {!card.kalos_job_number && (
          <span className="text-xs text-rose-500 font-medium ml-auto">No Job #</span>
        )}
      </div>
    </button>
  )
