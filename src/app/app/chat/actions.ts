...
export async function ensureBaseChannels(): Promise<void> {
  ...
  const { data: existing } = await supabase.from('chat_channels').select('kind, trade').eq('company_id', companyId)
...
export async function listMessages(input: { channelId: string; before?: string; after?: string; limit?: number }): Promise<ChatMessage[]> {
  ...
  const { data } = await q
  const rows = (data ?? []).map((m) => mapMessage(m as Record<string, unknown>))
...
export async function sendMessage(input: { channelId: string; body: string; asUpdate?: boolean }) {
  try {
    ...
    const { data: msg, error } = await supabase.from('chat_messages').insert({
      ...
      kind: 'message', project_id: channel.project_id, mentions,
    }).select('*').single()
    ...
    if (input.asUpdate && channel.kind === 'project' && channel.project_id) {
      ...
      if (feed) {
        const { data: m2 } = await supabase.from('chat_messages').insert({
          ...
          kind: 'update', project_id: channel.project_id, source_message_id: msg.id, mentions,
        }).select('*').single()
        if (m2) mirrored = mapMessage(m2 as Record<string, unknown>)
      }
    ...
    return { ok: true as const, message: { ...mapMessage(msg as Record<string, unknown>), kind: input.asUpdate && channel.kind === 'project' ? 'update' as const : 'message' as const }, mirrored, pinged: targets.size }
...
export async function deleteMessage(input: { id: string }) {
  try {
    const { supabase } = await ctx()
    const { error } = await supabase.from('chat_messages').update({ deleted_at: new Date().toISOString(), body: '' }).eq('id', input.id)
    if (error) return { ok: false as const, error: error.message }
    return { ok: true as const }
  } catch (e) { return fail(e) }
}
