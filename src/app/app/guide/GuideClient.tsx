...
const SECTIONS: GuideSection[] = [
  ...
  },
  {
    ...
    items: [
      { heading: 'Creating & editing', text: 'Projects capture name, client, job location, start/end dates, project manager, superintendent, subcontractors (add as many as needed), permit status (Not Required / Pending / Submitted / Approved / Denied), priority (Low → Critical), a trade/division, notes, color, and board placement. Which fields appear depends on the board\'s field customization.' },
      { heading: 'Trade filter (premium)', text: 'On Business and Enterprise plans, a trade switcher sits in the top bar. Pick a trade (Refrigeration, HVAC, Electrical…) and the whole app — Dashboard, Projects, Gantt, Boards, and Change Orders — scopes to that trade\'s projects until you switch back to All trades. Assign each project\'s trade in its form; the switcher remembers your choice across visits.' },
  ...
  },
  {
    ...
    items: [
      ...
      { heading: 'Revisions', text: 'Each sheet keeps its full history. The newest upload becomes CURRENT and older ones are marked SUPERSEDED — opening an old revision shows an unmissable orange banner with one tap back to current. Old revisions are never deleted automatically.' },
      { heading: 'Overlay & compare', text: 'In the viewer, the layers icon (top toolbar) opens the overlay bar. Pick what to lay over the current drawing — an older revision of the same sheet, OR any other drawing in the project (e.g. the mechanical sheet over the architectural). Choose Overlay to stack them with an opacity slider that fades between the two, or Side by side to view them together. Both drawings share the same pan/zoom, so lining up plans to spot differences is quick.' },
  ...
  },
  {
    ...
    items: [
      ...
      { heading: 'Crew roster', text: 'Each team has its own crew list under the top bar. Type a name to add someone, tap the ✕ on a chip to remove them. Tap the name itself to rename or swap in a different person — every day they are already scheduled on (this week and past/future weeks for that team) follows the change, so you never have to rebuild a week over a typo or a last-minute substitution. On phones and tablets the ✕ stays visible since there is no hover.' },
      { heading: 'Jobs & the project list', text: '"Add job" creates a blank job block for the week. Faster: keep your recurring projects (name + Job#) in the Projects panel on the left — one click drops a project onto the current team\'s week, and its Job# links to your job site (set the link pattern in Settings → Scheduling). When you add a project you choose which department(s) can see it: All (the default — every department), or one specific department. The ‹ › arrows above the list let you peek at another department\'s projects without leaving your team. Add or delete entries anytime.' },
      ...
      { heading: 'Job# links', text: 'For the Job# to be clickable on the schedule (and in the copied email), set your job-tracker URL once under Settings → Scheduling → "Job link on the weekly schedule". Paste the URL with {job} where the number goes (e.g. https://…/track?workorder={job}); every Job# then links to its own job. Leave it blank for plain numbers.' },
      { heading: 'Department schedule style', text: 'Each department can use one of two layouts, chosen from the style dropdown next to the department selector (managers only). "Crew grid" is the default — one block per job with day rows and tap-to-assign crew chips. "Startup grid" is a jobs-down / days-across table where each cell holds one or more people with a shift note (Days, Nights, Travel Day, As needed) — ideal for startup crews that move person-by-person rather than as a fixed crew. On the Startup grid, tap a name already in a cell to change the person or shift (Replace) or pull them off that day (Remove); the ✕ beside each name works on touch too. Both styles copy-for-email and print. Set it once per department and every team in it uses that layout.' },
