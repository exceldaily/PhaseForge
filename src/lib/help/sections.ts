...
export const SECTIONS: GuideSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { heading: 'Shift note colours', text: 'Each shift note carries its own colour, and every name scheduled with that note is drawn in it. Open any cell picker, hit “Add or remove shift notes”, and pick a colour from the row of swatches before you add a note, or tap a swatch under an existing note to recolour it. The colour is a department setting, so every team under it matches, and it follows through to the printed sheet and the copied email. A note you never colour keeps the sensible default: travel red, nights purple, as-needed amber, everything else green.' },
      { heading: 'Highlighting a row', text: 'On the Startup grid every job has a highlighter beside its delete button. Tint the row yellow, green, blue, red, purple, or orange to make it jump out — a job that needs attention, a crew that changed, whatever you need people to notice. The tint replaces the grey banding on that row and, importantly, survives Print and Copy for email, so the person reading it on paper or in Outlook sees exactly what you flagged. Switching a row back to normal works three ways: tap the colour it already has, use the crossed-out swatch in the palette, or hit “Back to normal” underneath. The row returns to the standard grey banding everywhere, including the next print and the next email.' },
