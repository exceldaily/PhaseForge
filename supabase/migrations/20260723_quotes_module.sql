...
CREATE TABLE IF NOT EXISTS public.quote_vendor_sends (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  quote_request_id uuid NOT NULL REFERENCES public.quote_requests(id) ON DELETE CASCADE,
  vendor_id uuid NOT NULL REFERENCES public.quote_vendors(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS quote_vendors_company_idx ON public.quote_vendors (company_id);
CREATE INDEX IF NOT EXISTS quote_requests_company_status_idx ON public.quote_requests (company_id, status);
CREATE INDEX IF NOT EXISTS quote_sends_company_idx ON public.quote_vendor_sends (company_id);
CREATE INDEX IF NOT EXISTS quote_sends_quote_idx ON public.quote_vendor_sends (quote_request_id);

CREATE TRIGGER user_gmail_accounts_updated BEFORE UPDATE ON public.user_gmail_accounts
  FOR EACH ROW EXECUTE FUNCTION public.quotes_set_updated_at();
CREATE TRIGGER quote_vendors_updated BEFORE UPDATE ON public.quote_vendors
  FOR EACH ROW EXECUTE FUNCTION public.quotes_set_updated_at();
CREATE TRIGGER quote_requests_updated BEFORE UPDATE ON public.quote_requests
  FOR EACH ROW EXECUTE FUNCTION public.quotes_set_updated_at();
CREATE TRIGGER quote_vendor_sends_updated BEFORE UPDATE ON public.quote_vendor_sends
  FOR EACH ROW EXECUTE FUNCTION public.quotes_set_updated_at();

ALTER TABLE public.user_gmail_accounts ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.quote_vendors ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.quote_requests ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.quote_vendor_sends ENABLE ROW LEVEL SECURITY;

-- Tokens: strictly personal.
CREATE POLICY "uga_own" ON public.user_gmail_accounts FOR ALL
  USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

-- Quotes data: shared across the company.
CREATE POLICY "qv_company" ON public.quote_vendors FOR ALL
  USING (company_id = public.get_my_company_id()) WITH CHECK (company_id = public.get_my_company_id());
CREATE POLICY "qr_company" ON public.quote_requests FOR ALL
  USING (company_id = public.get_my_company_id()) WITH CHECK (company_id = public.get_my_company_id());
CREATE POLICY "qvs_company" ON public.quote_vendor_sends FOR ALL
  USING (company_id = public.get_my_company_id()) WITH CHECK (company_id = public.get_my_company_id());
