...
DROP POLICY IF EXISTS "attachments read" ON public.project_attachments;
CREATE POLICY "attachments read" ON public.project_attachments FOR SELECT
  USING (
    project_id IN (
...
DROP POLICY IF EXISTS "attachments insert" ON public.project_attachments;
CREATE POLICY "attachments insert" ON public.project_attachments FOR INSERT
  WITH CHECK (
...
DROP POLICY IF EXISTS "attachments update" ON public.project_attachments;
CREATE POLICY "attachments update" ON public.project_attachments FOR UPDATE
  USING (
    project_id IN (
      SELECT id FROM projects
      WHERE company_id IN (SELECT company_id FROM profiles WHERE id = auth.uid())
...
DROP POLICY IF EXISTS "attachments delete" ON public.project_attachments;
CREATE POLICY "attachments delete" ON public.project_attachments FOR DELETE
  USING (
    project_id IN (
      SELECT id FROM projects
      WHERE company_id IN (SELECT company_id FROM profiles WHERE id = auth.uid())
